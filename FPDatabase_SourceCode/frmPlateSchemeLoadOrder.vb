	...
	Private Sub btnSaveLoadOrder_Click(ByVal eventSender As System.Object, ByVal eventArgs As System.EventArgs) Handles btnSaveLoadOrder.Click
        ...
        For lRow = 0 To flexPlate.RowCount - 1
            For lColumn = 0 To flexPlate.ColumnCount - 1
                rowName = Chr(Asc("A") + lRow)
                LoadOrderMap.Parameters("load_order_id").Value = lLoadOrderId
                LoadOrderMap.Parameters("counter").Value = flexPlate.Rows(lRow).Cells(lColumn).Value
                LoadOrderMap.Parameters("well_row").Value = rowName
                LoadOrderMap.Parameters("well_column").Value = lColumn + 1
...
btnSaveLoadOrder_Error: 
    ...
    Private Sub flexPlate_ClickEvent(ByVal eventSender As System.Object, ByVal eventArgs As System.EventArgs) Handles flexPlate.Click
        Dim lRow As Integer
        Dim lColumn As Integer
        ...
        If flexPlate.SelectedCells.Count > 0 And flexPlate.SelectedCells(0).Value.ToString().Trim() = "" Then
            flexPlate.SelectedCells(0).Value = lCounter
            lCounter = lCounter + 1
        End If

        'need to loop over all the cells and check if they are all filled in....

        For lRow = 1 To flexPlate.RowCount - 1
            For lColumn = 1 To flexPlate.ColumnCount - 1
                If flexPlate.Rows(lRow).Cells(lColumn).Value.ToString().Trim() = "" Then
                    bEmptyCell = True
                End If
            Next lColumn
        Next lRow
	...
	Private Sub fillPlateHeaders(ByVal PlateFormat As String)
		...
		If PlateFormat = "96" Then
            ...
            For lRow = 0 To flexPlate.RowCount - 1
                rowName = Chr(Asc("A") + lRow)
                flexPlate.Rows(lRow).HeaderCell.Value = rowName
            Next lRow
